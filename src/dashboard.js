...
(function () {
  ...
  function indexData() {
    ...
    EXERCISE_INDEX = buildExerciseIndex(SESSIONS);
  }

  function buildExerciseIndex(sessions) {
    const index = {};
    sessions.forEach((s, sidx) => {
      s.exercises.forEach((ex) => {
        ...
        index[key].equipment_options.add(ex.equipment || '—');
        index[key].instances.push({
          date: s.date,
          sessionIdx: sidx,
          sets: ex.sets,
          notes: ex.notes || [],
    ...
    Object.values(index).forEach((ex) => {
  ...
  function withinDays(dateISO, days) {
    return withinDaysFrom(dateISO, days, referenceDateISO());
  }
  function withinDaysFrom(dateISO, days, anchorISO) {
    const today = new Date(anchorISO + 'T00:00:00'); today.setHours(0,0,0,0);
    const d = new Date(dateISO + 'T00:00:00');
  ...
  function detectPRs(index) {
    index = index || EXERCISE_INDEX;
    const prs = []; // { date, exercise, type, value, set }
    for (const ex of Object.values(index)) {
      let bestE = 0;
      let bestW = 0;
  ...
  function forecastExercise(key, index) {
    index = index || EXERCISE_INDEX;
    const ex = index[key];
    if (!ex) return null;
  ...
  function allMuscles() {
    return [...new Set(Object.values(EXERCISE_INDEX).map((e) => e.muscle_group || 'Other'))].sort();
  }
  function allEquipment() {
    const vals = new Set();
    Object.values(EXERCISE_INDEX).forEach((e) => e.equipment_options.forEach((x) => vals.add(x || '—')));
    return [...vals].sort();
  }
  function allExercises() {
    return Object.values(EXERCISE_INDEX).slice().sort((a, b) => exerciseLabel(a).localeCompare(exerciseLabel(b)));
  }

  function activeDateRange() {
    if (!SESSIONS.length) return { start: '', end: '', label: 'All dates' };
    const first = SESSIONS[0].date;
    const last = SESSIONS[SESSIONS.length - 1].date;
    if (FILTERS.date === 'custom') {
      return {
        start: FILTERS.start || first,
        end: FILTERS.end || last,
        label: `${FILTERS.start || first} → ${FILTERS.end || last}`,
      };
    }
    if (FILTERS.date !== 'all') {
      ...
      return { start: start.toISOString().slice(0, 10), end: last, label: `Last ${days} days` };
    }
    return { start: first, end: last, label: 'All dates' };
  }

  function matchesText(session, ex, q) {
    if (!q) return true;
    const sessionBlob = [
      session.date,
      session.day_of_week,
    ...
    return sessionBlob.includes(q) || exBlob.includes(q);
  }

  function exercisePassesFilters(ex) {
    if (FILTERS.muscle !== 'all' && ex.muscle_group !== FILTERS.muscle) return false;
    if (FILTERS.exercise !== 'all' && exerciseKey(ex) !== FILTERS.exercise) return false;
    if (FILTERS.equipment !== 'all' && (ex.equipment || '—') !== FILTERS.equipment) return false;
    return true;
  }
  ...
  function getFilteredSessions() {
    ...
    return SESSIONS
      ...
      .map((s) => {
        const exercises = s.exercises.filter((ex) => exercisePassesFilters(ex) && matchesText(s, ex, q));
        return exercises.length ? { ...s, exercises } : null;
      })
      .filter(Boolean);
  }
  function sessionExerciseKeySet(sessions) {
    const keys = new Set();
    sessions.forEach((s) => {
      s.exercises.forEach((ex) => keys.add(`${s.date}|${exerciseKey(ex)}`));
    });
    return keys;
  }

  function sessionVolume(s) {
    return s.exercises.reduce((a, ex) => a + totalVolume(ex.sets), 0);
  }
  function sessionSetCount(s) {
    return s.exercises.reduce((a, ex) => a + ex.sets.length, 0);
  }
  function weeklyVolumeMap(sessions) {
    const map = {};
    for (const s of sessions) {
      const w = weekStart(s.date);
      map[w] = (map[w] || 0) + sessionVolume(s);
    }
    return map;
  }
  function exerciseStats(ex) {
    ...
    const volume = ex.instances.reduce((a, inst) => a + totalVolume(inst.sets), 0);
    const sets = ex.instances.reduce((a, inst) => a + inst.sets.filter((s) => !s.warmup).length, 0);
    const top = ex.instances
      .flatMap((i) => i.sets)
      .filter((s) => s.type === 'weighted' && !s.warmup)
      .reduce((best, s) => (!best || s.e1rm > best.e1rm ? s : best), null);
    return { first, last, trend, volume, sets, top };
  }
  ...
  function sortSessions(sessions, sort) {
    ...
    else if (sort === 'volume') out.sort((a, b) => sessionVolume(b) - sessionVolume(a) || byDate(b, a));
    else if (sort === 'sets') out.sort((a, b) => sessionSetCount(b) - sessionSetCount(a) || byDate(b, a));
    else out.sort((a, b) => byDate(b, a));
    return out;
  }

  function sortExercises(exercises, sort) {
    const out = exercises.slice();
    const stats = new Map(out.map((ex) => [ex.key, exerciseStats(ex)]));
    const byName = (a, b) => exerciseLabel(a).localeCompare(exerciseLabel(b));
    if (sort === 'name') out.sort(byName);
    else if (sort === 'best') out.sort((a, b) => stats.get(b.key).last - stats.get(a.key).last || byName(a, b));
    else if (sort === 'trend') out.sort((a, b) => stats.get(b.key).trend - stats.get(a.key).trend || byName(a, b));
    else if (sort === 'volume') out.sort((a, b) => stats.get(b.key).volume - stats.get(a.key).volume || byName(a, b));
    else out.sort((a, b) => b.instances.length - a.instances.length || byName(a, b));
    return out;
  }

  function sortPRs(prs, sort) {
    const out = prs.slice();
    if (sort === 'exercise') out.sort((a, b) => a.exercise.localeCompare(b.exercise) || b.date.localeCompare(a.date));
    else if (sort === 'type') out.sort((a, b) => a.type.localeCompare(b.type) || b.date.localeCompare(a.date));
    else if (sort === 'value') out.sort((a, b) => b.value - a.value || b.date.localeCompare(a.date));
    else out.sort((a, b) => b.date.localeCompare(a.date));
    return out;
  }

  function filterSummary(sessions) {
    const sets = sessions.reduce((a, s) => a + sessionSetCount(s), 0);
    const volume = sessions.reduce((a, s) => a + sessionVolume(s), 0);
    const range = activeDateRange();
    const parts = [range.label, `${sessions.length} sessions`, `${fmt(sets)} sets`, `${fmt(volume)} lb`];
    if (FILTERS.muscle !== 'all') parts.push(FILTERS.muscle);
    if (FILTERS.equipment !== 'all') parts.push(FILTERS.equipment);
    if (FILTERS.exercise !== 'all' && EXERCISE_INDEX[FILTERS.exercise]) parts.push(exerciseLabel(EXERCISE_INDEX[FILTERS.exercise]));
        ...
        <div class="filter-grid">
          <label>Date
            <select id="gf-date">
              ...
              <option value="30" ${FILTERS.date === '30' ? 'selected' : ''}>Last 30 days</option>
              <option value="60" ${FILTERS.date === '60' ? 'selected' : ''}>Last 60 days</option>
              <option value="90" ${FILTERS.date === '90' ? 'selected' : ''}>Last 90 days</option>
              <option value="custom" ${FILTERS.date === 'custom' ? 'selected' : ''}>Custom</option>
            </select>
          </label>
          <label>From
            <input type="date" id="gf-start" value="${escapeHTML(FILTERS.start || range.start)}" ${FILTERS.date === 'custom' ? '' : 'disabled'} />
          </label>
          <label>To
            <input type="date" id="gf-end" value="${escapeHTML(FILTERS.end || range.end)}" ${FILTERS.date === 'custom' ? '' : 'disabled'} />
          </label>
          <label>Muscle
            <select id="gf-muscle">
              <option value="all" ${FILTERS.muscle === 'all' ? 'selected' : ''}>All muscles</option>
              ${allMuscles().map((m) => `<option value="${escapeHTML(m)}" ${FILTERS.muscle === m ? 'selected' : ''}>${escapeHTML(m)}</option>`).join('')}
            </select>
          </label>
          <label>Exercise
            <select id="gf-exercise">
              <option value="all" ${FILTERS.exercise === 'all' ? 'selected' : ''}>All exercises</option>
              ${exerciseOptions.map((ex) => `<option value="${escapeHTML(ex.key)}" ${FILTERS.exercise === ex.key ? 'selected' : ''}>${escapeHTML(exerciseLabel(ex))}</option>`).join('')}
            </select>
          </label>
          <label>Equipment
            <select id="gf-equipment">
              <option value="all" ${FILTERS.equipment === 'all' ? 'selected' : ''}>All equipment</option>
              ${allEquipment().map((eq) => `<option value="${escapeHTML(eq)}" ${FILTERS.equipment === eq ? 'selected' : ''}>${escapeHTML(eq)}</option>`).join('')}
            </select>
          </label>
          <label class="filter-search">Search
            <input type="search" id="gf-search" placeholder="Exercise, note, template..." value="${escapeHTML(FILTERS.q)}" />
          </label>
          <button class="btn-ghost" id="gf-reset">Reset</button>
        </div>
        <div class="filter-summary">${escapeHTML(filterSummary(sessions))}</div>
      </section>
    `;
  }
  ...
  function attachFilterBar() {
    ...
    document.getElementById('gf-start').addEventListener('change', (e) => {
      FILTERS.date = 'custom';
      FILTERS.start = e.target.value;
      refreshCurrentView();
    });
    document.getElementById('gf-end').addEventListener('change', (e) => {
      FILTERS.date = 'custom';
      FILTERS.end = e.target.value;
      refreshCurrentView();
    });
    document.getElementById('gf-muscle').addEventListener('change', (e) => {
      FILTERS.muscle = e.target.value;
      refreshCurrentView();
    });
    document.getElementById('gf-exercise').addEventListener('change', (e) => {
      FILTERS.exercise = e.target.value;
      refreshCurrentView();
    });
    document.getElementById('gf-equipment').addEventListener('change', (e) => {
      FILTERS.equipment = e.target.value;
      refreshCurrentView();
    });
    document.getElementById('gf-search').addEventListener('input', (e) => {
      FILTERS.q = e.target.value;
      clearTimeout(window.__gf);
      window.__gf = setTimeout(refreshCurrentView, 180);
    });
    document.getElementById('gf-reset').addEventListener('click', () => {
      FILTERS = { date: 'all', start: '', end: '', muscle: 'all', exercise: 'all', equipment: 'all', q: '' };
      refreshCurrentView();
    });
  }
  ...
  function renderOverview() {
    ...
    const fullVol = SESSIONS.reduce((a, s) => a + sessionVolume(s), 0);
    const fullSets = SESSIONS.reduce((a, s) => a + sessionSetCount(s), 0);
    const fullFirstDate = SESSIONS[0].date;
    const fullLastDate = SESSIONS[SESSIONS.length - 1].date;
    const fullWeekMap = weeklyVolumeMap(SESSIONS);
    const fullWeeks = Object.keys(fullWeekMap).sort();
    if (!filteredSessions.length) {
      app.innerHTML = `
        <div class="dashboard-screen">
          ${trainerIntroHTML(fullFirstDate, fullLastDate, allSessions, fullSets, fullVol)}
          <div class="section-head">
            <h1>Overview</h1>
            <span class="sub">No matching training data</span>
          </div>
          ${renderFilterBar(filteredSessions)}
          <div class="empty">No sessions match the current filters.</div>
        </div>
        ${printReportHTML({ firstDate: fullFirstDate, lastDate: fullLastDate, totalSessions: allSessions, totalSets: fullSets, totalVol: fullVol, weekMap: fullWeekMap, weeks: fullWeeks })}
      `;
      attachFilterBar();
    ...
    const totalVol = filteredSessions.reduce((a, s) => a + sessionVolume(s), 0);
    const totalSets = filteredSessions.reduce((a, s) => a + sessionSetCount(s), 0);
    const sessionsPerWeek = (totalSessions / (totalDays / 7)).toFixed(1);

    // Last-30-day stats
    const recent = filteredSessions.filter((s) => withinDaysFrom(s.date, 30, lastDate));
    const recentVol = recent.reduce((a, s) => a + sessionVolume(s), 0);
    ...
    // PRs (recent)
    const prSort = CURRENT.params.prSort || 'newest';
    const visibleExerciseDates = sessionExerciseKeySet(filteredSessions);
    const allPRs = detectPRs();
    const recentPRs = sortPRs(allPRs
      .filter((p) => visibleExerciseDates.has(`${p.date}|${p.exerciseKey}`) && withinDaysFrom(p.date, 30, lastDate))
    , prSort).slice(0, 6);
    ...
    app.innerHTML = `
      <div class="dashboard-screen">
        ...
        <div class="section-head">
          <h1>Overview</h1>
          <span class="sub">${pretty(firstDate)} → ${pretty(lastDate)} · ${fmt(totalDays)} days in view</span>
        </div>
        ${renderFilterBar(filteredSessions)}
        ...
        <div class="grid cols-2" style="margin-bottom:18px;">
          <div class="panel">
            <div class="panel-title-row">
              ...
              <select id="pr-sort" class="compact-select" aria-label="Sort PRs">
                <option value="newest" ${prSort === 'newest' ? 'selected' : ''}>Newest</option>
                <option value="value" ${prSort === 'value' ? 'selected' : ''}>Value</option>
                <option value="exercise" ${prSort === 'exercise' ? 'selected' : ''}>Exercise</option>
                <option value="type" ${prSort === 'type' ? 'selected' : ''}>Type</option>
              </select>
            </div>
            ${recentPRs.length === 0
    ...
    // Heatmap
    renderHeatmap(document.getElementById('heatmap-host'), filteredSessions, lastDate);
    ...
    attachFilterBar();
    const prSortEl = document.getElementById('pr-sort');
    if (prSortEl) prSortEl.addEventListener('change', (e) => {
      CURRENT.params.prSort = e.target.value;
      refreshCurrentView();
    });
    const printBtn = document.getElementById('print-summary');
    if (printBtn) printBtn.addEventListener('click', () => window.print());
  ...
  function renderHeatmap(host, sessions, anchorDate) {
    sessions = sessions || SESSIONS;
    if (!sessions.length) return;
    const today = new Date((anchorDate || referenceDateISO()) + 'T00:00:00'); today.setHours(0,0,0,0);
    const days = 365;
  ...
  function renderHistory() {
    const sort = CURRENT.params.sort || 'newest';
    const filtered = sortSessions(getFilteredSessions(), sort);

    app.innerHTML = `
      <div class="section-head">
        <h1>History</h1>
        <span class="sub">${filtered.length} sessions in view · click a card to expand</span>
      </div>
      ...
      <div class="toolbar">
        <select id="history-sort">
          ...
          <option value="volume" ${sort === 'volume' ? 'selected' : ''}>Highest volume</option>
          <option value="sets" ${sort === 'sets' ? 'selected' : ''}>Most sets</option>
        </select>
      </div>
    ...
    document.getElementById('history-sort').addEventListener('change', (e) => {
      CURRENT.params.sort = e.target.value;
      refreshCurrentView();
    });
  }
  ...
  function renderExercises() {
    const sort = CURRENT.params.sort || 'sessions';
    const scopedIndex = buildExerciseIndex(getFilteredSessions());
    const exs = sortExercises(Object.values(scopedIndex), sort);
      ...
      return `
        <div class="ex-row" data-name="${escapeHTML(ex.key)}">
          ...
          <div>
            ...
            <div class="stat-value">${stats.top ? `${fmt(stats.top.weight)} × ${stats.top.reps}` : '—'}</div>
          </div>
          <div>
    ...
    app.innerHTML = `
      ...
      ${renderFilterBar(getFilteredSessions())}
      <div class="toolbar">
        <select id="exercise-sort">
          <option value="sessions" ${sort === 'sessions' ? 'selected' : ''}>Most sessions</option>
          <option value="best" ${sort === 'best' ? 'selected' : ''}>Best e1RM</option>
          <option value="trend" ${sort === 'trend' ? 'selected' : ''}>Best trend</option>
          <option value="volume" ${sort === 'volume' ? 'selected' : ''}>Most volume</option>
          <option value="name" ${sort === 'name' ? 'selected' : ''}>Name</option>
        </select>
      </div>
      ${rows ? `<div class="ex-list">${rows}</div>` : '<div class="empty">No exercises match those filters.</div>'}
    `;
    attachFilterBar();
    document.getElementById('exercise-sort').addEventListener('change', (e) => {
      CURRENT.params.sort = e.target.value;
      refreshCurrentView();
    });
  ...
  function renderExerciseDetail(name) {
    const filteredSessions = getFilteredSessions();
    const scopedIndex = buildExerciseIndex(filteredSessions);
    const ex = scopedIndex[name];
    if (!ex) {
      app.innerHTML = `
        <a href="#" class="back-btn" id="back-ex">← All exercises</a>
        ${renderFilterBar(filteredSessions)}
        <div class="empty">Exercise not found in the current filters.</div>
      `;
      attachFilterBar();
  ...
  function renderForecast(name) {
    ...
    const filteredSessions = getFilteredSessions();
    const scopedIndex = buildExerciseIndex(filteredSessions);
    const exs = sortExercises(Object.values(scopedIndex)
      .filter((e) => e.instances.some((i) => bestE1rm(i.sets) > 0))
    , sort);
    if (!name || !scopedIndex[name]) name = exs[0] ? exs[0].key : null;
    ...
    if (!name) return;
    renderForecastFor(name, scopedIndex);
  }

  function renderForecastFor(name, index) {
    const host = document.getElementById('forecast-host');
    const fc = forecastExercise(name, index);
    if (!fc || fc.message) {
